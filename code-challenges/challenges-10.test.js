...
Build a simple express server. Connect a '/hello' route that sends a greeting of your choice. Connect an '/aboutme' route that sends a short bio about you to the front-end. Finally, connect a '/favoritefoods' route that sends an array to the front-end of your favorite foods. All other routes should respond with a status of 404.
------------------------------------------------------------------------------------------------ */
...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.send('A twin and the youngest of fifteen children, Nathan currently finds himself in the relative quiet of a small family in a small town. Nathan is fluent in Russian and is keen on adding coding to that short list of foreign languages.')
      .catch(error => {
        res.status(504).send(error);
      });
  });
  ...
  app.get('/favoritefoods', (req, res) => {
    let favoriteFoods = ['Chicken Paprika', 'Banana Cream Pie', 'Red Wine', 'Coffee', 'Celery Juice'];
...
const count = (target, input) => {
  ...
  let counter = 0;
  input.map(number => {
    number.map(val => val === target ? counter++ : counter);
  });
  return counter;
};
