...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(starWarsChar => {
    ...
    addPeople.html($htmlTemplate);
    addPeople.find('h2').text(starWarsChar.name);
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  array.splice(str.length - (numberOfCharacters), numberOfCharacters);
  return array.join('');
};
...
const removeVowels = (str) => {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i] === 'a' || array[i] === 'e' || array[i] === 'i' || array[i] === 'o' || array[i] === 'u') array.splice(i, 1);
  }
  return array.join('');
};
